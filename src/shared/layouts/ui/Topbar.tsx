...
import { getPageTitleKey } from "@/shared/config/navigation"
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 shrink-0 items-center gap-3 border-b border-border/60 bg-background/80 px-4 backdrop-blur-xl sm:px-6">
      ...
      <div className="relative ml-auto hidden max-w-sm flex-1 md:block">
        ...
        <Input placeholder={t('common.search')} className="bg-muted/50 pl-9" />
      </div>
      ...
      <div className="ml-auto flex items-center gap-2 md:ml-0">
        ...
        <Button asChild size="icon" className="sm:hidden">
          <Link to={ROUTES.TRANSACTIONS} aria-label={t('common.addTransaction')}>
            <Plus className="size-4" />
          </Link>
        </Button>
